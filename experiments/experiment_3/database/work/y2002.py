# coding: utf-8
from datetime import datetime
from snowballing.models import *
from ..places import *

althoff2002a = DB(WorkUnrelated(
    2002, "The indiGo project: enhancement of experience management and process learning with moderated discourses",
    display="althoff",
    authors="Althoff, Klaus-Dieter and Becker-Kornstaedt, Ulrike and Decker, Björn and Klotz, Andreas and Leopold, Edda and Rech, Jörg and Voss, Angi",
    place=FAKE,
    pp="53--79",
    entrytype="incollection",
    publisher="Springer",
    ID="althoff2002indigo",
    springer2016="1",
    placex="Advances in Data Mining",
))

bennetts2002a = DB(WorkUnrelated(
    2002, "Measurement of performance and role play in Churchmanian design systems: an instantiation",
    display="bennetts",
    authors="Bennetts, Peter and Wood-Harper, Trevor",
    place=FAKE,
    pp="228",
    entrytype="article",
    ID="bennetts2002measurement",
    gs2016="1",
    placex="AMCIS 2002 Proceedings",
))

cmusei2002a = DB(WorkUnrelated(
    2002, "Capability Maturity Model",
    display="cmu sei",
    authors="CMU SEI",
    place=FAKE,
    other1="Version 1.1",
    other2="CMMI SM for Software Engineering (CMMI-SW/SE/IPPD/SS, V1.1) Continuous Representation",
    placex="Integration (CMMISM)",
))

conradi2002a = DB(WorkUnrelated(
    2002, "Improving software process improvement",
    display="conradi",
    authors="Conradi, R., Fuggetta, A.",
    place=IEEES,
    placex="IEEE Software",
))

dolado2002a = DB(WorkUnrelated(
    2002, "Apoyo a la decisión en Ingeniería del Software.",
    display="dolado",
    authors="Dolado, José Javier and Cuadrado, JJ and García, D Rodríguez",
    place=FAKE,
    pp="11",
    entrytype="inproceedings",
    ID="dolado2002apoyo",
    gs2016="1",
    placex="JISBD",
))

kitchenham2002a = DB(WorkUnrelated(
    2002, "Preliminary guidelines for empirical research in software engineering",
    display="kitchenham",
    authors="Kitchenham, Barbara A and Pfleeger, Shari Lawrence and Pickard, Lesley M and Jones, Peter W and Hoaglin, David C. and El Emam, Khaled and Rosenberg, Jarrett",
    place=ToSE,
    pp="721--734",
    entrytype="article",
    volume="28",
    number="8",
    publisher="IEEE",
    ID="kitchenham2002preliminary",
    gs2016="1",
    placex="IEEE Transactions on software engineering",
))

middleton2002a = DB(WorkUnrelated(
    2002, "Information Systems Analysis",
    display="middleton",
    authors="Middleton, Michael and others",
    place=FAKE,
    pp="366",
    entrytype="article",
    publisher="Charles Sturt University, Centre for Information Studies",
    ID="middleton2002information",
    gs2016="1",
    placex="Information Management: A Consolidation of Operations, Analysis and Strategy",
))

ruiz2002a = DB(WorkUnrelated(
    2002, "An XMI-based repository for software process meta-modeling",
    display="ruiz",
    authors="Ruiz, Francisco and Piattini, Mario and García, Félix and Polo, Macario",
    place=ICPFSPI,
    pp="546--558",
    entrytype="inproceedings",
    organization="Springer",
    ID="ruiz2002xmi",
    cluster_id="15405828224327782001",
    scholar="http://scholar.google.com/scholar?cites=15405828224327782001&as_sdt=2005&sciodt=0,5&hl=en",
    gs2016="1",
    placex="International Conference on Product Focused Software Process Improvement",
    springer2016="1",
    volume="2559",
    link="https://www.scopus.com/inward/record.uri?eid=2-s2.0-84949946857&partnerID=40&md5=2db3e294fb79fbbd65b6d2e14e09642c",
    scopus2016_duplicate_removed="1",
))

schleicher2002a = DB(WorkUnrelated(
    2002, "Infering Process Model Definition Knowledge",
    display="schleicher",
    authors="Schleicher, Ansgar",
    place=FAKE,
    pp="197--228",
    entrytype="incollection",
    publisher="Springer",
    ID="schleicher2002infering",
    gs2016="1",
    placex="Management of Development Processes",
    springer2016="1",
))
